...
def _subject_id_from_filename(stem: str) -> str:
    """Extract subject id from filenames like S01_Training, S01_Validation."""
    # Keep only the leading token that looks like a subject id (e.g. S01, 1, subj1)
    tokens = stem.replace("_", " ").split()
    if tokens:
        tok = tokens[0]
        # e.g. S01, S01_Training -> S01
        return tok
    return stem
